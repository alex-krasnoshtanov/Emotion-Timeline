...
## The classification unit is a fixed chunk
...
failing at the end of a run. The small container relies on that: `:study` carries
no transcriber, so it opens the page, draws the committed example, and disables
the run button with a line saying why.

The two published images, and the cards they run on, are
[their own chapter](container.md).
...
- **The valence band is a reading, not a measurement.** It was scored on
  ...
  scenes the label cannot separate, and no further than that.
