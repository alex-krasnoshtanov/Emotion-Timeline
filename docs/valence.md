...
0.6 / 0.8. The model was never anyone's guess — it is
...
**Valence works. Arousal barely does** — 0.5734 where 0.5 is nothing at all. So
...
0.4816 — 212 rows right, 130 wrong, p < 0.0001**, which looks like the combination
rule [the Russian chapter](russian.md) says does not exist.

It is not. Balanced accuracy, which ignores the class prior, goes **0.4559 →
...
— which pays on a Neutral-heavy test set and would be harmful on a documentary
with a different mix. The chapter's finding stands, and the record keeps the
numbers so the next person does not have to re-derive them to be disappointed.
...
- **That arousal is useless.** It is weak *at ranking this project's seven
  ...
  text-only model may simply be the wrong instrument for it — the audio would be
  the right one, and [the cut list](../README.md#scope) says why that is not here.
- **That a better VA model would not help the label.** Only this one was tested.
  Its base and large checkpoints agreed closely when the base was chosen — the
  figures are in the [release notes](https://github.com/alex-krasnoshtanov/Emotion-Timeline/releases/tag/weights-va-v1),
  not in a benchmark here, because that was a selection decision rather than a
  published result. It is weak evidence that capacity is not the binding
...
(MIT), mirrored as `weights-va-v1` with a recorded SHA-256 — repackaged as
