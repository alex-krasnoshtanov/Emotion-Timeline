...
- **The dataset build reproduces the record exactly, or fails.** `compare()` is
  not approximate: the source corpus is a fixed published artefact and every step
...
- **Bugs in the original build are reproduced, not fixed.** The emoticon
  ...
  would make this repository describe a dataset nobody trained on. Document it,
  quantify it, and leave it for a retrain that can measure the difference.
...
docs/                 one chapter per stage, plus container.md; each ends in
                      what it does NOT show
assets/               figures, regenerated by `emotion-timeline figures`
tests/                the arithmetic, and the mistakes worth pinning
Dockerfile            two targets: `study` (core + web) and `app` (everything)
compose.yaml          both of them, published to loopback
```

The images install the project into `/app` rather than into site-packages,
because every read-only command finds its records relative to its own file. A
wheel on its own looks for `<venv>/Lib/benchmarks/...`, which is why there is no
PyPI package. `.github/workflows/release.yml` builds both images, runs
`timeline` inside each and checks the numbers, then pushes to GHCR.
